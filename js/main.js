...
window.addEventListener("DOMContentLoaded", function () {
  if (document.querySelector(".footer-year")) {
    document
      ...
      .forEach((el) => (el.textContent = new Date().getFullYear().toString()));
  }
});
...
(function () {
  ...
  if (multilevelOffcanvas) {
    multilevelOffcanvas.querySelectorAll(".offcanvas__subnav").forEach((el) => {
  ...
  for (const option of document.querySelectorAll(
    ...
    option.addEventListener("click", function () {
      if (!this.classList.contains("selected")) {
        ...
        select.querySelector(".custom-select-field div").textContent =
          this.textContent;
        ...
        if (this.getAttribute("data-info")) {
          document.querySelector("#role-info").textContent =
            this.getAttribute("data-info");

          if (selected === "1") {
            document
              .querySelectorAll(".form-additional")
          ...
          if (selected !== "0") {
            document
              .querySelector(".form-registration-fields")
              .classList.add("show");
          } else {
            document
              .querySelector(".form-registration-fields")
...
<path d="M41.5295 6.53033C41.8224 6.23744 41.8224 5.76256 41.5295 5.46967L36.7565 0.696699C36.4636 0.403806 35.9888 0.403806 35.6959 0.696699C35.403 0.989593 35.403 1.46447 35.6959 1.75736L39.9385 6L35.6959 10.2426C35.403 10.5355 35.403 11.0104 35.6959 11.3033C35.9888 11.5962 36.4636 11.5962 36.7565 11.3033L41.5295 6.53033ZM0.427734 6.75H40.9992V5.25H0.427734V6.75Z" fill="#1E1E1E"/>
  ...
  $("#signup-calendar").on("zabuto:calendar:day", function (e) {
    $("td.zabuto-calendar__day--today").removeClass(
...
if (document.querySelector(".categories-list")) {
  document.querySelectorAll(".categories-list a").forEach((link) => {
    link.addEventListener("click", () => {
      document.querySelectorAll(".categories-list a").forEach((link2) => {
...
if (document.querySelector(".service-card")) {
  ...
  document.querySelectorAll(".service-card").forEach((card) => {
    let cardBodyHeight = card.querySelector(".service-card-body").clientHeight;
    let btnHeight =
      card.querySelector(".service-card-btn").parentElement.clientHeight;
    let sectionHeight = document.querySelector('.services-wrapper').clientHeight
		document.querySelector('.services-wrapper').style.maxHeight=sectionHeight-5+'px'
    card.querySelector(".service-card-body").style.height =
      cardBodyHeight - btnHeight - 5 + "px";
...
if (document.querySelector(".promo-detail-sidebar")) {
  const promoDetailSidebar = document.querySelector(".promo-detail-sidebar");
  const footer = document.querySelector("footer");
  const promoDetailSidebarWidth = promoDetailSidebar.clientWidth;
  let lastScrollY = window.scrollY;
  let isScrollingUp = false;
  const sidebarTop = promoDetailSidebar.offsetTop;

  window.addEventListener("scroll", () => {
    if (window.innerWidth > 991) {
      const scrollY = window.scrollY;
      const sidebarHeight = promoDetailSidebar.clientHeight;
      const windowHeight = window.innerHeight;
      const footerTop = footer.offsetTop;

      if (scrollY >= sidebarTop && scrollY + windowHeight < footerTop + 103) {
        promoDetailSidebar.style.position = "fixed";
        promoDetailSidebar.style.top = "0";
        isScrollingUp = false;
        promoDetailSidebar.style.maxWidth = promoDetailSidebarWidth + "px";
      } else if (scrollY <= sidebarTop && isScrollingUp) {
        promoDetailSidebar.style.position = "relative";
      } else if (scrollY + windowHeight >= footerTop + 103) {
        promoDetailSidebar.style.position = "absolute";
        promoDetailSidebar.style.top = `${footerTop - sidebarHeight - 64}px`;
        isScrollingUp = false;
        promoDetailSidebar.style.maxWidth = promoDetailSidebarWidth + "px";
        ...
        promoDetailSidebar.style.position = "static";
        promoDetailSidebar.style.maxWidth = promoDetailSidebarWidth + "px";
      }
      ...
      promoDetailSidebar.style.maxWidth = "100%";
    }
  });
}
